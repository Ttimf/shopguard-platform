...
- ✅ **Этап 15.3 — Analytics UI** (Flutter UI поверх data-слоя 15.2; backend/архитектура не тронуты).
  - **Аудит UI перед стартом:** переиспользованы `AppColors`/`buildTheme`, Material (`Card`/`ListTile`/`Chip`/
    `RefreshIndicator`/`AppBar`/`DropdownButtonFormField`), паттерн `BlocProvider→BlocBuilder`, стиль приватных
    карточек (как `_AlertCard`). Найдено: переиспользуемых виджетов состояний не было (Empty/Loading приватно
    дублировались, Error+Retry отсутствовал) → вынесены в `core/`.
  - **6 экранов** (`features/analytics/ui/`): Events (фильтры период+eventType, pull-to-refresh),
    Tracking (сегменты сгруппированы по `trackingId` → маршрут cam→cam), Behavior (риск-скор с выделением
    высокого риска рамкой+прогресс-баром), Purchases (MATCHED/PARTIAL/NOT_MATCHED цветом + missing/extra items),
    Alert Decisions (severity LOW/MEDIUM/HIGH/CRITICAL цветом), AI Models (список + активная модель, только
    просмотр). Хаб `AnalyticsHubPage` встроен 5-й вкладкой в `home_shell` (навигация не сломана).
  - **Новые универсальные компоненты** (`core/`, всё на базе темы/Material):
    - `core/cubit/list_cubit.dart` — обобщённые `ListState<T>` (Equatable) + `ListCubit<T>` (один класс на все
      движки; `run(fetch)` → loading/success/error). Без per-экранного дублирования cubit'ов.
    - `core/widgets/async_list_view.dart` — `AsyncListView<T>`: Loading/Empty/Error+Retry/Success + pull-to-refresh.
    - `core/widgets/status_badge.dart` (`StatusBadge`), `core/widgets/info_chip.dart` (`InfoChip` — обобщает
      `_chip` из camera_card; camera_card переключён на него, дубль удалён).
    - `core/error/error_mapper.dart` — `friendlyError()` (401/403/404/409/422/5xx/timeout/no-internet → RU).
    - `features/analytics/ui/analytics_style.dart` — маппинг статус→цвет/подпись (реальные enum backend).
  - **API (только существующие):** Events `GET /api/events` · Tracking `GET /api/tracks` · Behavior
    `GET /api/behavior` · Purchases `GET /api/purchases` · Alert Decisions `GET /api/alerts` · AI Models
    `GET /api/models`. Новые endpoint не выдумывались. DI: общий `DioClient`, новых DioClient/TokenStorage/
    Singleton нет.
  - **Зафиксировано (нет в API — не выдумано):** `suspiciousCount` (Behavior) в DTO/API отсутствует → показаны
    имеющиеся `productsTaken/productsReturned`; per-model «статус загрузки» (AI Models) в API нет → используется
    состояние загрузки экрана, «версия» = имя модели.
  - **Проверки:** `flutter clean`/`pub get`/`analyze` — **0 проблем**; `flutter test` — **20 passed** (7 новых +
    13 прежних); `flutter build web` — **успешно** (√ Built build\web; предупреждения wasm-dry-run относятся к
    `flutter_secure_storage_web`, не к коду этапа).
  - **Регрессий нет:** backend не менялся; существующие экраны работают (analyze/тесты/сборка зелёные), новая
    вкладка добавлена без слома навигации; дублирование устранено (InfoChip).

- ✅ **Этап 15.4 — Camera Management & Live Monitoring** (Flutter поверх существующих camera-API; backend не тронут).
  - **Аудит перед стартом:** CameraRepository/CameraModel/CamerasCubit/CameraCard/cameras_page — здоровые,
    переиспользованы. `CameraStatusValue` = ONLINE|OFFLINE|UNKNOWN → ложится на green/yellow/red. `GET /api/cameras/:id`
    существует. WebSocket (`alerts.gateway`) шлёт **только** алерты (AlertCreated), статусы камер не пушит →
    по ТЗ оставлен **pull-to-refresh** (уже был), новые WS не создавались.
  - **Список камер (доработан card):** цвет-индикатор green/yellow/red, бейджи статуса и **AI работает/стоп**,
    resolution/FPS/последнее соединение, **подсказка об ошибке** для OFFLINE/UNKNOWN, tap → детали. Существующие
    поиск/фильтр/тест/превью/enable-toggle/CRUD — без изменений.
  - **Детальная страница (новая):** статус + AI + **модель AI** + последнее соединение; **проверка соединения**
    (кнопка, состояния Loading/Success/Error — latency/fps/resolution/ошибка); полные параметры; **последняя
    активность** (последнее событие Event Engine по камере); **история ошибок** (события `CameraOffline`).
    Состояния Loading/Empty/Error+Retry/Success + pull-to-refresh.
  - **API (только существующие):** `GET /api/cameras`, `GET /api/cameras/:id`, `POST /api/cameras/:id/test` (проверка),
    `GET /api/events?cameraId=…` (активность + история ошибок), `GET /api/stores` + `GET /api/models` (модель AI).
    Новые endpoint не выдумывались.
  - **Зафиксировано (нет в API — выведено, не выдумано):** `aiRunning` — геттер `enabled && ONLINE` (отдельного поля
    нет); «модель AI» — `store.modelOverride ?? models.default` (у камеры поле `model` = аппаратная модель);
    «последнее обнаружение/активность» и «история ошибок» — из Event Engine (`CameraOffline`), т.к. отдельного
    лога ошибок камеры в API нет; latency — только из результата `test`.
  - **Переиспользовано:** CameraRepository (+1 метод `getCamera`), CameraModel (+геттер `aiRunning`), CamerasCubit,
    EventsEngineRepository/StoreRepository/AiModelsRepository (15.2), `StatusBadge`/`InfoChip`/`friendlyError` (15.3),
    общий `DioClient`/DI. Новых DioClient/TokenStorage/Singleton нет.
  - **Новое:** `CameraDetailCubit` (+Equatable-стейт), `camera_detail_page.dart`, `camera_status.dart` (цвет/подпись/
    подсказка), `core/widgets/state_message.dart` (`StateMessage` — вынесен из приватного `_Message` в AsyncListView,
    переиспользован; дублирование устранено).
  - **Проверки:** `flutter clean`/`pub get`/`analyze` — **0 проблем**; `flutter test` — **24 passed** (4 новых +20);
    `flutter build web` — **успешно** (√ Built build\web; предупреждения wasm-dry-run — `flutter_secure_storage_web`).
  - **Регрессий нет:** backend/REST/архитектура не менялись; существующие экраны работают; список камер расширен
    обратимо; дублирование состояний устранено (StateMessage).

- ✅ **Этап 15.5 — Store Management & Application Settings** (Flutter поверх существующих API; backend не тронут).
  - **Аудит перед стартом:** Profile/Settings(Telegram)/AuthCubit/CameraManagement — переиспользованы. Найдено:
    `GET /api/health` есть (без версии backend), `GET /api/me` есть, `apiBaseUrl` — в dio_client. Store DTO:
    id/name/address/telegramChatId/modelOverride/createdAt.
  - **Store (новый экран):** название, адрес, владелец (из AuthCubit), число камер (N + онлайн M — из
    CameraRepository), AI-модель (`modelOverride ?? default`), дата создания, статус (производный: онлайн/всего).
    Loading/Empty/Error+Retry/Success + pull-to-refresh.
  - **AI Settings (карточка на Store):** текущая модель/версия (= имя), модель магазина (override или «по
    умолчанию»), модель по умолчанию (`GET /api/models`). Только просмотр (редактирования нет).
  - **App Settings (новый экран «О приложении»):** версия приложения (`core/app_info.dart`), **версия backend —
    «недоступно (нет в API)»** (честно зафиксировано), сервер (`apiBaseUrl`), статус подключения (из AuthCubit),
    пользователь (имя/email/роль) + переход в Диагностику.
  - **Diagnostics (новый экран):** Backend доступен / API отвечает (`GET /health`), Авторизация активна (токен в
    TokenStorage), Токен действителен (`GET /auth/me`), число камер (`GET /cameras`), число моделей (`GET /models`).
    Каждая проверка изолирована (сбой одной не рушит остальные); pull-to-refresh + повтор.
  - **Profile (улучшен):** имя/email/роль/магазин (магазин — FutureBuilder по StoreRepository) + навигация к
    Store/Telegram/О приложении + существующая кнопка «Выйти» (`AuthCubit.logout`).
  - **API (только существующие):** `GET /api/stores`, `GET /api/cameras`, `GET /api/models`, `GET /api/health`,
    `GET /api/auth/me`, `PATCH /api/stores/:id/telegram` (уже был). Новых endpoint нет.
  - **Ограничения backend (зафиксированы, не выдуманы):** версии backend в API нет (health отдаёт service/status/
    time); отдельного поля `owner`/`status`/«число камер» в Store DTO нет (владелец берётся из сессии, статус/счётчик
    выводятся из камер); `GET /api/stores/:id` нет (используется список).
  - **Переиспользовано:** StoreRepository/CameraRepository/AiModelsRepository/AuthRepository(+метод `me`),
    AuthCubit, `apiBaseUrl`, `StatusBadge`/`InfoChip`/`StateMessage`/`friendlyError`, общий `DioClient`/DI.
    Новых DioClient/TokenStorage/Singleton нет.
  - **Новое универсальное:** `core/widgets/labeled_row.dart` (`LabeledRow` — строка ключ-значение; переиспользована
    в Store/AppSettings/Diagnostics; `camera_detail._InfoCard` переведён на неё — дублирование устранено);
    `core/app_info.dart` (`appVersion`). Cubit'ы: `StoreCubit`, `DiagnosticsCubit` (Equatable).
  - **Проверки:** `flutter clean`/`pub get`/`analyze` — **0 проблем**; `flutter test` — **26 passed** (2 новых +24);
    `flutter build web` — **успешно** (√ Built build\web; предупреждения wasm-dry-run — `flutter_secure_storage_web`).
  - **Регрессий нет:** backend/REST/архитектура не менялись; существующие экраны работают; дублирование устранено
    (LabeledRow); Profile улучшен без слома навигации/logout.

- ✅ **Этап 15.6 — Production Readiness** (аудит + точечные фиксы; backend не тронут; только mobile).
  - **UX-аудит:** проверены Loading/Empty/Error/Retry/Success на всех экранах. Найдены **2 реальных бага**:
    `alerts_page` и `history_page` при сетевой ошибке показывали «пусто» вместо ошибки (в alerts ещё и
    маскировалось под «создайте магазин»). **Исправлено** — Error+Retry через общий `StateMessage`. Остальные
    экраны (аналитика/камеры/детали/store/diagnostics/login/settings) уже корректны — без изменений.
  - **Release-конфиг (production-фиксы):**
    - **AndroidManifest: добавлен `<uses-permission INTERNET>`** — критично: в release не добавляется
      автоматически, без него APK не имел бы доступа к API. **Label `shopguard_mobile` → `ShopGuard`**.
    - iOS `CFBundleDisplayName` → `ShopGuard`; web `index.html`/`manifest.json` (title/name/short_name/description/
      theme_color `#0F172A`) → ShopGuard; `pubspec.description` → реальное. **bundle id/applicationId уже реальные**
      (`com.shopguard.*`) — не трогал.
  - **Navigation/Performance/Network/Security-аудит (проверено, фиксов не требуют):** переходы (hub→движки,
    profile→store/settings/appsettings/diagnostics, cameras→detail, login↔logout) целостны; `AlertsCubit.close()`
    закрывает socket+subscription, контроллеры/AnimationController — с dispose; Events-cubit создаётся полем и
    закрывается в dispose; refresh-on-401 с ротацией/повтором; `friendlyError` покрывает 401/403/404/500/timeout/
    no-net → без падений; токены только в `flutter_secure_storage`; **debug-print/секретов/TODO в коде нет**.
  - **Code-аудит (SOLID/DRY/KISS/YAGNI):** мёртвого кода/неиспользуемых импортов нет (`analyze` = 0); дублирование
    ранее устранено (`InfoChip`/`StateMessage`/`LabeledRow`). Новых компонентов не создавал.
  - **Проверки:** `flutter clean`/`pub get`/`analyze` — **0 проблем**; `flutter test` — **26 passed**;
    `flutter build web` — **√**; `flutter build apk --release` — **√ app-release.apk 51.5 MB**.
  - **Оставшиеся ограничения (зафиксированы, не блок кода):** иконки приложения — дефолтные Flutter (нужны
    дизайн-ассеты перед стором); release APK подписан debug-ключом (для Play нужен keystore/`key.properties` —
    секреты держать вне git); версия приложения `1.0.0+1` дублируется в `core/app_info.dart` вручную
    (package_info_plus намеренно не добавляли); версии backend в API нет (см. 15.5).
  - **Регрессий нет:** backend/архитектура не менялись; все сборки/тесты зелёные; исправлены только реальные UX-баги
    и release-конфиг.

- ✅ **RTSP Integration Readiness — аудит цепочки + минимальные фиксы** (ai-detection; без нового функционала,
  без смены архитектуры). Подготовка к тестам с реальными RTSP-камерами.
  - **Аудит цепочки** (`camera_worker`, `rtsp_probe`, `main`-оркестратор): подключение/переподключение (backoff),
    CameraOffline/Online, освобождение `cap`, таймауты, логирование, устойчивость, утечки, зависания ffmpeg/OpenCV.
  - **Найдены и исправлены 4 реальные проблемы (минимально):**
    1. **Поток воркера мог упасть от любого исключения** (в цикле не было try/except) → `cap` не освобождался,
       камера терялась. Фикс: внутренний `try/except` (сбой кадра логируется, поток продолжает; при поломке `cap`
       следующий `read()` уходит в переподключение) + внешний `try/finally` — `cap.release()` гарантирован.
    2. **Оркестратор не перезапускал «мёртвый» воркер** (`sync()` делал `continue`, т.к. id оставался в словаре) →
       камера терялась навсегда до смены конфига/рестарта. Фикс: `not worker.is_alive()` → удаление и пересоздание.
    3. **`self._seen` рос безгранично** (каждый новый track_id, без очистки) → медленная утечка памяти при долгой
       работе. Фикс: кап `SEEN_TRACK_CAP` (по умолч. 5000) с очисткой (track_id монотонно растут — повторов почти нет).
    4. **Не было явных таймаутов OpenCV** (полагались на устаревшую ffmpeg-опцию `stimeout`) → риск зависания на
       недоступной/зависшей камере. Фикс: общий `rtsp_probe.open_capture()` с `CAP_PROP_OPEN/READ_TIMEOUT_MSEC`
       (по умолч. 10с), переиспользован в воркере и в тесте/снапшоте (устранено дублирование конструктора capture).
  - **Проверено (без изменений):** экспоненциальный backoff min→max; CameraOffline/Online по факту потери/возврата;
    `rtsp_probe.test_stream`/`grab_frame` уже освобождают `cap` через `finally`; `_stop.wait()` — прерываемый сон;
    `_buffer` (deque maxlen), `_blacklist_last`/`_embed_cache` — ограничены; события — только публикация (без БД/push).
  - **Тесты:** новый `tests/test_rtsp_resilience.py` (2): устойчивость воркера к ошибке кадра + гарантированный
    `cap.release()` + кап `_seen`; перезапуск мёртвого воркера оркестратором (оба — офлайн, фейковый capture/детектор).
    **Итого 35 passed** (33 прежних + 2). Регрессий нет.
  - **Остаточные риски:** реальная проверка с физической RTSP-камерой (сеть/задержки/вендоры) — только на стенде;
    `CAP_PROP_*_TIMEOUT_MSEC` применяются на уровне OpenCV/FFmpeg (эффективность зависит от сборки ffmpeg — но
    подтверждено: невалидный хост падает быстро, без зависания).

- ✅ **End-to-End Integration Verification** (проверка живого стека; код/архитектура/контракты не менялись).
  Проверена вся цепочка RTSP→AI→Redis→5 движков→Notification→Gateway→Mobile на работающих 11 контейнерах.
  - **Redis Streams / Consumer Groups (пункты 1–5,10):** `events.stream` (XLEN 72) — **5 групп** (event-engine,
    tracking-engine, behavior-engine, purchase-matching, alert-engine), у каждой `consumers=1`, `pending=0`,
    `lag=0`, `entries-read=72`, одинаковый `last-delivered-id` → фан-аут без потерь и без повторов.
    `detection.events` (XLEN 5) — группа `notification`, `pending=0`, `lag=0`. `XPENDING=0`.
  - **Персистентность (пункт 2):** EventLog=72 (= XLEN = entries-read всех групп), PersonTrack=12,
    BehaviorSession=17, PurchaseSession=7, AlertDecision=2 (THEFT_ALERT/HIGH + CAMERA_OFFLINE/MEDIUM), Event=2.
    Причинная цепочка в логе: CameraOffline→SuspiciousActivity→AlertCreated.
  - **Восстановление после рестарта (пункт 6):** `docker restart event` → healthy за ~15с, все 5 движков
    перезапустились чисто; **EventLog остался 72, pending/lag=0** → возобновление с `last-delivered-id`, **без
    повторной обработки/дублей**.
  - **Камера off/reconnect (пункт 7):** ai-detection лог — недоступные RTSP (`cam.invalid`, `10.255.255.1:554`)
    → **быстрый отказ без зависания ffmpeg + backoff 60с**; CameraOffline зафиксирован, при возврате будет
    CameraOnline (RTSP-устойчивость подтверждена ранее unit-тестами).
  - **Gateway→сервисы→Mobile (пункт 8):** `/api/health` 200; auth-поток (register→JWT→`/api/auth/me` вернул
    пользователя); `/api/models` → реальные данные `{available:[v1,v2], default:v1}` через gateway→сервис.
  - **Race / память (пункты 9,10):** по одному consumer на группу + XACK, `pending=0` стабильно (в т.ч. после
    рестарта) → гонок нет; Redis память **1.5M** (пик 1.73M), стримы ограничены — накопления нет.
  - **Docker Compose с нуля (пункт 11):** `docker-compose.yml` валиден (`config -q`) и подтверждён работающим
    холодным стартом (все 11 контейнеров healthy); прод-оверлей `+docker-compose.prod.yml --env-file .env.production`
    валиден (12 сервисов с Caddy).
  - **Реальных ошибок интеграции не найдено → правок кода не вносилось** (рабочий код не трогал).
  - **Примечания:** развёрнутый образ ai-detection предшествует RTSP-хардненинг-правкам (они в исходниках; при
    `docker compose up -d --build ai-detection` попадут в контейнер — вне рамок этой проверки); для auth-теста
    созданы 2 throwaway-пользователя (`e2e*@test.local`) — безвредные тестовые записи.
